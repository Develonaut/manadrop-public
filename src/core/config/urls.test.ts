  ...
  test("getUrl() should return a constructed url friendly string", () => {
    const urls = [
      ...
      },
      {
        test: {
          url: "test/:testParam",
          pathArgs: { testParam: "test with chars" },
          queryParams: { queryParamKey: "query+param+key" },
          encode: true
        },
    ...
    urls.forEach(
      ({ test: { url, pathArgs, queryParams, encode }, expected }) => {
        expect(getUrl({ url, pathArgs, queryParams, encode })).toBe(expected);
      }
    );
  });
});
